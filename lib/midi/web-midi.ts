...
import { MIDINoteEvent, MIDIConnectionStatus, MIDIInputDevice, MIDILatencyConfig } from '@/lib/types/midi';

class MIDIManager {
    private midiAccess: MIDIAccess | null = null;
    private activeInput: MIDIInput | null = null;
    private noteCallback: ((event: MIDINoteEvent) => void) | null = null;
    private latencyConfig: MIDILatencyConfig = {
        enabled: true,
        offsetMs: 25 // Default 25ms compensation for MIDI hardware latency
    };

    constructor() {
        // Load latency config from localStorage on initialization
        this.loadLatencyConfig();
    }

    private loadLatencyConfig(): void {
        try {
            const stored = localStorage.getItem('midiLatencyConfig');
            if (stored) {
                this.latencyConfig = JSON.parse(stored);
            }
        } catch (error) {
    ...
    setLatencyCompensation(offsetMs: number): void {
        this.latencyConfig.offsetMs = offsetMs;
        this.latencyConfig.lastCalibrated = new Date();
        // Persist to localStorage
        try {
            localStorage.setItem('midiLatencyConfig', JSON.stringify(this.latencyConfig));
        } catch (error) {
            console.warn('Failed to save MIDI latency config:', error);
        }
    }

    getLatencyCompensation(): MIDILatencyConfig {
    ...
    private handleMIDIMessage(message: MIDIMessageEvent): void {
        ...
        // Calculate compensated timestamp (anticipate by offsetMs to correct for hardware latency)
        const compensatedTimestamp = this.latencyConfig.enabled
            ? message.timeStamp - this.latencyConfig.offsetMs
            : message.timeStamp;
