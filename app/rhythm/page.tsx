...
export default function RhythmPage() {
    ...
    // Interaction Handler (Touch/Click or MIDI)
    const handleInteraction = useCallback((midiEvent?: MIDINoteEvent) => {
        if (!isPlaying) return;

        // Use compensated timestamp for MIDI input, audio time for touch/click
        const currentTime = midiEvent?.compensatedTimestamp
            ? midiEvent.compensatedTimestamp / 1000  // Convert ms to seconds
            : getAudioTime();
        ...
        setActiveNotes(currentNotes => {
            ...
            const PERFECT_WINDOW_S = 0.10; // 100ms
            const GOOD_WINDOW_S = 0.15;   // 150ms
